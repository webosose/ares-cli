...
Generator.prototype.existOutDir = function(outDir) {
    ...
    try {
        ...
        if (err && err.code === 'ENOTDIR') {
            throw errHndl.changeErrMsg("NOT_DIRTYPE_PATH", outDir);
        }
...
Generator.prototype.generate = function(options) {
    ...
    if (!template) {
        return promise.reject(errHndl.changeErrMsg("INVALID_TEMPLATE"));
    }
    ...
    function _writeAppIDdata(qmlTmpl, appId) {
        ...
        return promise.all(filePaths.map(function(file) {
            return fs.lstatAsync(file)
                .then(function(stats) {
                    if (!stats.isFile()) {
                        throw errHndl.changeErrMsg("INVALID_PATH", "meta template", file);
                    }
