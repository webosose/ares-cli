...
Generator.prototype.generate = function(options) {
    ...
    function _writeAppIDdata(qmlTmpl, appId) {
        ...
        .then( function() {
            log.silly("generator#generate()#_writeAppIDdata()", "done");
            return;
        })
        .catch( function(err) {
            log.verbose("generator#generate()#_writeAppIDdata()", "err:", err);
            throw err;
        });
    }
    ...
    function _writeURLdata(urlTmpl, url) {
        ...
        .then( function() {
            log.silly("generator#generate()#_writeURLdata()", "done");
            return;
        })
        .catch( function(err) {
            log.verbose("generator#generate()#_writeURLdata()", "err:", err);
            throw err;
        });
    }
    ...
    function _writeMetadata(metaTmpl, _appinfo, _svcinfo, _pkginfo) {
        ...
        .catch( function(err) {
            log.verbose("generator#generate()#_writeMetadata()", "err:", err);
            throw err;
        });
    }
};
