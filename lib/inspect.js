            ...
            function _findNewDebugPort(dbgPort, next) {
                ...
                function _onData(data) {
                    ...
                    if (str === "0") {
                        ...
                        return next(errHndl.changeErrMsg("FAILED_GET_PORT"));
                    }
                }
            }
                    ...
                    async.waterfall([
                        function findSvcFilePath(next) {
                            ...
                            if (!options.svcDbgInfo[serviceId].path) {
                                return next(errHndl.changeErrMsg("FAILED_GET_SVCPATH", serviceId));
                            }
                            next();
                        },
                        function parserMeta(next) {
                            ...
                            async.series([
                                ...
                                if (err) {
                                    return next(errHndl.changeErrMsg("FAILED_FIND_SERVICE", serviceId));
                                }
                            });
                        ...
                        function checkServiceType(metaData, next) {
                            try {
                                ...
                                if (metaInfo.engine === "native") {
                                    return next(errHndl.changeErrMsg("USE_GDB", serviceId));
                                }
                                next();
                            } catch (err) {
                                next(err);
                        ...
                        for (const index in pagelist) {
                            if (pagelist[index].url.indexOf(options.appId) !== -1 ||
                                ...
                                if (!pagelist[index][listFile.propName]) {
                                    return next(errHndl.changeErrMsg("USING_WEBINSPECTOR"));
                                }
