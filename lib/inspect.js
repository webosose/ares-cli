    ...
    const inspector = {
        ...
        inspect: function(options, params, next) {
            ...
            async.series([
                ...
                function(next) {
                    log.info("inspector#inspect()", "running...");
                    setImmediate(next);
                }
            ], function(err, results) {
                log.silly("inspector#inspect()", "err: ", err, ", results:", results);
            ...
            function _runApp(next) {
                ...
                launcher.listRunningApp(options, function(err, runningApps) {
                    ...
                    log.info("inspector#inspect()#_runApp()", "runAppIds:", runAppIds.join(','));
            ...
            function _findNewDebugPort(dbgPort, next) {
                ...
                function _onData(data) {
                    ...
                    if (str === "0") {
                        log.silly("inspector#inspect()#_findNewDebugPort()", "final dbgPort : " + dbgPort);
                        next(null, dbgPort);
                    } else if (str === "1") {
            ...
            function _runServicePortForwardServer(next) {
                ...
                function __eachServicePortForward(serviceId, next) {
                    ...
                    log.info("inspector#inspect()#_eachServicePortForward()", "sessionId : " + options.sessionId + ", default dbgPort : " + dbgPort);
                    ...
                    async.waterfall([
                        function findSvcFilePath(next) {
                            log.info("inspector#inspect()#findSvcFilePath()");
                            spinner.start();
                            if (options.instPkgs) {
                        ...
                        function runService(next) {
                            log.info("inspector#inspect()#runService()", "serviceId :", serviceId);
                            const param = {},
                                addr = {
                                    "service": serviceId,
                                    "method": "info"
