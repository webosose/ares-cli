...
(function() {
    ...
    function createLogFile(options, next) {
        ...
        fs.open(options.argv.argv.remain[0], 'w', function(err, fd) {
            if (err) {
        ...
        log.verbose("log#createLogFile()", options.argv.argv.remain[0] + " is exist: " + fs.existsSync(options.argv.argv.remain[0]));
        next();
    }
