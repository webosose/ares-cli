    ...
    const logLib = {
        ...
        checkLogDaemon: function (options, next) {
            ...
            async.series([
                ...
                if (err) {
                    console.log("Current log daemon is " + options.currentDaemon +" on CLI");
                    next(err, null);
                } else {
                    next(null, {msg : "Current log daemon is " + options.currentDaemon
                                        +" on CLI\nThe "+options.currentDaemon+" is active in target device"});
                }
            });
