...
(function() {
    ...
    const logLib = {
        ...
        show: function(options, next) {
            ...
            function _createLogFile(next) {
                log.verbose("log#_createLogFile()");
                if (options.argv.save) {
        ...
        readMode: function(options, next) {
            ...
            function _generateCmd(next) {
                ...
                log.info("log#_generateCmd()", "options.cmd:" + options.cmd);
                next();
            }
    ...
    function makeSession(options, next){
        log.info("log#makeSession()", (options.session ? "already session connected" : "need to make new session"));
        if (!options.session) {
    ...
    function generateCmd(options, next) {
        if (options.currentDaemon === "pmlogd") {
            ...
            log.info("log#generateCmd()", "options.cmd:", options.cmd);
        }
        next();
    }
    ...
    function createLogFile(options, next) {
        ...
        log.info("log#createLogFile()", "options.argv.argv.remain[0]:" + options.argv.argv.remain[0]);
        next();
    }
