...
(function() {
    ...
    const installer = {
        ...
        install: function(options, hostPkgPath, next) {
            ...
            if (!hostPkgPath) {
                return next(errHndl.changeErrMsg("EMPTY_VALUE", "PACKAGE_FILE"));
            }
            ...
            async.waterfall([
                ...
                function(next) {
                    if (options.opkg) {
                        ...
                        if (options.session.getDevice().username !== 'root') {
                            return setImmediate(next, errHndl.changeErrMsg("NEED_ROOT_PERMISSION", "opkg install"));
                        }
                    }
                ...
                function(next)	{
                    if (!srcMd5 || !dstMd5) {
                        ...
                        if (srcMd5 !== dstMd5) {
                            return next(errHndl.changeErrMsg("FAILED_TRANSMIT_FILE"));
                        }
                    }
                    next();
                },
                    ...
                    function _appinstalld(next) {
                        ...
                        luna.send(options, addr, param, function(lineObj, next) {
                            ...
                            if (resultValue.match(/FAILED/i)) {
                                ...
                                next(errHndl.getServiceErrMsg(addr.service, "FAILED_CALL_LUNA", errValue));
            ...
            async.waterfall([
                ...
                function(session, next) {
                    ...
                    if (options.opkg) {
                        // FIXME: Need more consideration whether this condition is necessary or not.
                        if (options.session.getDevice().username !== 'root') {
                            return setImmediate(next, errHndl.changeErrMsg("NEED_ROOT_PERMISSION","opkg remove"));
                        }
                    }
                    setImmediate(next);
                },
                    ...
                    function _opkg(next) {
                        ...
                        function __data(data) {
                            ...
                            if (str.match(/No packages installed or removed/g)) {
                                return next(errHndl.changeErrMsg("FAILED_REMOVE_PACKAGE", packageName));
                            } else {
                                console.log(str.trim());
                            }
                        }
            ...
            async.series([
                ...
                function(next) {
                    if (options.opkg) {
                        // FIXME: Need more consideration whether this condition is necessary or not.
                        if (options.session.getDevice().username !== 'root') {
                            return setImmediate(next, errHndl.changeErrMsg("NEED_ROOT_PERMISSION", "opkg list"));
                        }
                    }
                    setImmediate(next);
                },
                    ...
                    function _appinstalld(next) {
                        ...
                        luna.send(options, addr, param, function(lineObj, next) {
                            ...
                            if (Array.isArray(resultValue)) {
                                ...
                                next(errHndl.changeErrMsg("INVALID_OBJECT"));
                            }
