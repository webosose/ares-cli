        ...
        pull: function(options, next) {
            ...
            function _transferFiles(session, next) {
                ...
                try {
                            ...
                            session.run(cmd, null, writeIsDir, null,  function() {
                                ...
                                if (writeIsDir.getContentsAsString() === 'd\n') {
                                    ...
                                    try {
                                        ...
                                        if (!stat.isDirectory()) {
                                            return next(errHndl.changeErrMsg("NOT_DIRTYPE_PATH", destinationPath));
                                        }
                                    } catch(e) {
                                        if (e && e.code === 'ENOENT') {
                                    ...
                                    const err = errHndl.changeErrMsg("NOT_EXIST_PATH", "SOURCE", sourcePath);
                                    return setImmediate(next,err);
                                }
                            });
                        }
                    ...
                    let error;
                    if (err.code === 1) {
                        error = errHndl.changeErrMsg("INVALID_VALUE", "path", err);
                    }
                    finish(error);
                }
            }
