...
(function() {
    ...
    const device = {
        ...
        resourceSystem: function(options, next) {
            ...
            function _getSystemInfo(next) {
                ...
                if (!options.interval) {
                    try {
                        ...
                        clearTimeout(timerId);
                        log.silly("device#resourceSystem()#_getSystemInfo()", err.toString());
            ...
            function _setSystemInfo(systemData) {
                ...
                try {
                    ...
                    log.silly("device#resourceSystem()#_setSystemInfo()", err.toString());
                    return;
                }
            }
        ...
        /**
         * get running app and service CPU and memory usage status
         * @property options {String} device, interval
         */
        resourceProcessList: function(options, next) {
            ...
            function _getProcessInfo(next) {
                // If "interval" option is not given, call repeatProcessInfo() 2 times
                const defaultRepeat = 2;
                let repeatCount = 0,
                    timerId;
                if (!options.interval) {
                    try {
                        ...
                        clearTimeout(timerId);
                        log.silly("device#resourceSystem()#_getProcessInfo()", err.toString());
                        // ignore timer logic error
                        return next();
                    }
                } else {
            ...
            function _printProcessList(processgrpinfo) {
                log.info("device#resourceProcessList()#_setProcessInfo()",  JSON.stringify(processgrpinfo));
