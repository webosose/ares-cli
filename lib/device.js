        ...
        captureScreen: function(options, next) {
            ...
            async.series([
                ...
                _removeCaptureDir(function finish(removeErr){
                    // kepp the first error
                    if (err || removeErr){
                        next(err? err : removeErr);
                    } else {
                        let resultTxt = "";
                        resultTxt = "Create " + options.capFilename + " to " + options.destinationPath + "\n" + "Success";
                        next(null, resultTxt.trim());
                    }
                });
            });
            ...
            function _makeCaptureOption(next) {
                log.info("device#capture#_makeCapturePath()");
                let capFilename = "";
                let capFormat = "PNG"; // PNG is default format
                let outputDirpath;

                if (options.inputPath === null){
                    capFilename = _createDateFileName();
                    outputDirpath = path.resolve('.');
                } else {
                    // Get dirPaht, Filebase, fileExtenstion from given path
                    const parseDirPath = path.dirname(options.inputPath);
                    ...
                    if (parseBase && parseExt) {
                        ...
                        capFilename = _createDateFileName();
                        outputDirpath = path.resolve(path.join(options.inputPath));
                    }
                }

                options.capFormat = capFormat;
                options.capFilename = capFilename;
                options.capDirPath = "/tmp/capture",
                options.sourcePath = path.join(options.capDirPath, capFilename);
                options.destinationPath = outputDirpath;
                options.ignore = true; // Do not print detail messages during pull operation
            ...
            function _captureFile(next) {
                ...
                const cmd = "/bin/mkdir -p " + options.capDirPath;
                options.session.run(cmd, null, null, null, function(err){
                    if (err) {
                        ...
                        luna.sendWithoutErrorHandle(options, addr, param, function(lineObj, next) {
                            ...
                            if (lineObj.returnValue) {
                                console.log("Capture file in device : " + lineObj.output);
                                next(null, {});
                            } else {
                                return setImmediate(next, errHndl.changeErrMsg("FAILED_CALL_LUNA", lineObj.errorText), {});
                            }
                        }, next);
                    }
                });
            ...
            function _removeCaptureDir(next) {
                ...
                const cmd = '/bin/rm -rf ' + options.capDirPath;
                options.session.run(cmd, null, null, null, function(err){
                    if (err) {
                        return setImmediate(next, err);
                    } else {
