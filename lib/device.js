...
(function() {
    ...
    const device = {
         ...
         * @property options {String} device, display, inputPath
         */
        captureScreen: function(options, next) {
            ...
            async.series([
                ...
                let resultTxt = "";
                resultTxt = "Create " + chalk.green(options.capFilename) + " to " + options.destinationPath +"\nSuccess";

                // clean up /tmp/capture directory in target device
                if (options.targetCapDir) {
            ...
            function _makeCaptureOption(next) {
                ...
                if (options.inputPath === null){
                    ...
                    if (parseBase && parseExt) {
                        ...
                        outputDirPath = path.resolve(parseDirPath);
                    } else if (parseBase && (parseExt === "")) {
                        // it's directory path. Use it as destination dir Path
                        capFilename = _createDateFileName();
                        outputDirPath = path.resolve(options.inputPath);
                    }
                }
