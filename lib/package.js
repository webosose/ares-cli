...
(function () {
    ...
    Packager.prototype = {
        checkInputDirectories: function(inDirs, options, next) {
            log.info("Packager#checkInputDirectories()", "input directory:", inDirs);
    ...
    function loadAppInfo(next) {
        ...
        try {
            ...
            log.info("Packager#loadAppInfo()", "content of appinfo.json:", this.appinfo);
        ...
        async.waterfall([
            ...
            if (err) {
                ...
                if (result && result.errors.length > 0) {
                    ...
                    log.info("Packager#checkAppInfo()", "APPINFO is valid");
                }
                setImmediate(next);
            }
        });
    ...
    function createAppDir(next) {
        ...
        log.verbose("Packager#createAppDir()", "application dir:" + this.applicationDir);
    ...
    function checkELFHeader(next) {
        ...
        fs.read(fd, buf, 0, ELF_HEADER_LEN, 0, function(err, bytesRead, _buf) {
                if (bytesRead < ELF_HEADER_LEN || err) {
                    log.verbose("Packager#checkELFHeader()", "err:", err, ", bytesRead:", bytesRead);
                    log.verbose("Packager#checkELFHeader()", "readBuf to parse ELF header is small or error occurred during reading file.");
                    return setImmediate(next);
                }

                if (!_isELF(_buf)) {
                    log.verbose("Packager#checkELFHeader()", mainFile + " is not ELF format");
                } else {
                    log.verbose("Packager#checkELFHeader()", mainFile + " is ELF format");
                    try {
                        const elfHeader = elfParser.parseHeader(_buf);
                        log.verbose("Packager#checkELFHeader()", "elfHeader:", elfHeader);
    ...
    function copyApp(next) {
        ...
        this.dataCopyCount++;
        log.verbose("Packager#copyApp()", "copy " + this.appDir + " ==> " + this.applicationDir);
    ...
    function fillPackageDir(next) {
        if (!this.rom) {
            ...
            if (!this.pkgDir) {
                ...
                log.verbose("Packager#fillPackageDir()", "generating package.json: " + data);
    ...
    function outputPackage(destination, next) {
        log.verbose("Packager#outputPackage()");
    ...
    function makeIpk(srcDir, next) {
        ...
        log.verbose("Packager#makeIpk()", "makeIpk in dir " + this.IpkDir + " file " + this.ipkFileName);
    ...
    function loadServiceInfo(next) {
        ...
        log.verbose("Packager#loadServiceInfo()", "num of serviceInfo: " + this.services.length);
        setImmediate(next);
    }
    ...
    function addServiceInPkgInfo(next) {
        if (!this.rom) {
            ...
            try {
                ...
                log.verbose("Packager#addServiceInPkgInfo()", "PACKAGEINFO:", pkginfo);
            } catch (err) {
                console.error(err);
                setImmediate(next, err);
            }
            ...
            if (validServiceCount > 0) {
                ...
                log.verbose("Packager#addServiceInPkgInfo()", "Modified package.json: " + data);
                fs.writeFile(path.join(this.packageDir, "packageinfo.json"), data, next);
            } else {
                setImmediate(next);
            }
