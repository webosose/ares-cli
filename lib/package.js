        ...
        analyzeIPK: function(options, next) {
            ...
            function _unpackTar(next) {
                ...
                if (fs.existsSync(path.resolve(tmpDirPath, "control.tar.gz"))) {
                    tar.extract({
                        file: path.resolve(tmpDirPath, "control.tar.gz"),
                        cwd : tmpDirPath
                    }).then(function() {
                    ...
                    (async function() {
                        await decompress(path.resolve(tmpDirPath, "control.tar.xz"), tmpDirPath, {
                            plugins: [
                                decompressTarxz()
                            ]
                        });
                        log.silly('control.tar.xz decompressed');
                    })();
                } else {
                ...
                if (fs.existsSync(path.resolve(tmpDirPath, "data.tar.gz"))) {
                    tar.extract({
                        file: path.resolve(tmpDirPath, "data.tar.gz"),
                        cwd : tmpDirPath
                    }).then(function() {
                    ...
                    (async function() {
                        await decompress(path.resolve(tmpDirPath, "data.tar.xz"), tmpDirPath, {
                            plugins: [
                                decompressTarxz()
                            ]
                        });
                        log.silly('data.tar.xz decompressed');
                        next();
                    })();
                } else {
            ...
            function _analyzeMetaFile(next) {
                ...
                ipkConfig.webOSMetaFiles.forEach(function(item){
                    ...
                    if (item === "control") {
                        targetFile = path.resolve(tmpDirPath, ipkConfig[item].path, ipkConfig[item].fileName);
                    ...
                    } else if (fs.existsSync(path.resolve(tmpDirPath, ipkConfig[item].path))) {
                            const dirArr = fs.readdirSync(path.resolve(tmpDirPath, ipkConfig[item].path));
                            let beforeDir = dirArr[0];

                            dirArr.forEach(function(dir) {
                                if (ipkConfig[item].fileName) {
                                    ipkConfig[item].fileName.forEach(function(name) {
                                        targetFile = path.resolve(tmpDirPath, ipkConfig[item].path, dir, name);
                                        if (fs.existsSync(targetFile)) {
                                            if (options.infodetail) {
                                                result += "\n\n--- < " + name + " > ---\n";
                                                result += fs.readFileSync(targetFile).toString().trim();
                                ...
                                } else if (ipkConfig[item].dirs) { // Analyze meta files under usr/share/luna-service2
                                    targetFile = path.resolve(tmpDirPath, ipkConfig[item].path, dir);
                                    const fileName = fs.readdirSync(path.resolve(targetFile));
                                    let heading;

                                    fileName.forEach(function(name) {
                                        targetFile = path.resolve(targetFile, name);
                                        heading = path.join(dir, name);
                                        if (fs.existsSync(targetFile)) {
                                            if (options.infodetail) {
                                                result += "\n\n--- < " + heading + " > ---\n";
                                                result += fs.readFileSync(targetFile).toString().trim();
                                ...
                                } else { // Analyze built-in native service
                                    targetFile = path.resolve(tmpDirPath, ipkConfig[item].path);
                                    if (options.info && fs.existsSync(targetFile)) {
                                        result += "\n" + ipkConfig[item].heading + "\nid: ";
                                        fs.readdirSync(targetFile).forEach(function(file) {
                                            result += file + ", ";
                                        });
