...
(function () {
    ...
    function Packager(options) {
        ...
        log.verbose("Packager#packager()", "Packager id:" + this.objectId);
        this.appCount = 0;
        this.services = [];
    ...
    Packager.prototype = {
        checkInputDirectories: function(inDirs, options, next) {
            log.verbose("Packager#checkInputDirectories()", "input directory:", inDirs);
        ...
        servicePackaging: function(inDirs, destination, options, next) {
            log.info("Packager#servicePackaging()");
            async.series([
        ...
        generatePackage: function(inDirs, destination, options, next) {
            log.info("Packager#generatePackage()", "from ", inDirs);
    ...
    function loadPkgInfo(next) {
        ...
        if (fs.existsSync(this.pkginfofile)) {
            ...
            try {
                ...
                log.verbose("loadPkgInfo()", "PKGINFO:", this.pkginfo);
    ...
    function loadAppInfo(next) {
        ...
        try {
            ...
            log.silly("Packager#loadAppInfo()", "content of appinfo.json:", this.appinfo);
        ...
        async.waterfall([
            ...
            if (err) {
                ...
                if (result && result.errors.length > 0) {
                    ...
                    log.verbose("Packager#checkAppInfo()", "APPINFO is valid");
                }
                setImmediate(next);
            }
        });
    ...
    function createTmpDir(next) {
        ...
        log.verbose("Packager#createTmpDir()", "temp dir:", this.tempDir);
    ...
    function createAppDir(next) {
        ...
        log.info("Packager#createAppDir()", "application dir:" + this.applicationDir);
            ...
            try {
                async.forEachSeries(files, function(file, next) {
                    if (!FILE_TYPE[file.type]) {
                        log.verbose("Packager#copySrcToDst()#_copySrcToDst()", "ignore 'unknown file type'("+file.type+")");
                        return;
                    }

                    if (!file.relPath) {
                        log.verbose("Packager#copySrcToDst()#_copySrcToDst()", "ignore 'unknown path'");
                        return setImmediate(next);
                    }
                    ...
                    if (file.type === FILE_TYPE.symlink) {
                        ...
                        if (fs.existsSync(sourceFile)) {
                            if (minify && '.js' === path.extname(sourceFile) && file.relPath.indexOf('node_modules') === -1 ) {
                                log.verbose("Packager#copySrcToDst()#_copySrcToDst()", "require minification # sourceFile:", sourceFile);
                                try {
                                    ...
                                    log.verbose("Packager#copySrcToDst()#_copySrcToDst()", util.format('Failed to uglify code %s: %s', sourceFile, e.stack));
                            ...
                            log.verbose("Packager#copySrcToDst()#_copySrcToDst()", "ignore '" + file.relPath + "'");
                        }
                    }
                    setImmediate(next);
                }, function(err) {
    ...
    function checkELFHeader(next) {
        ...
        if (stats.size < ELF_HEADER_LEN) {
            log.verbose("Packager#checkELFHeader()", "file size is smaller than ELF Header size");
            return setImmediate(next);
        }

        fs.read(fd, buf, 0, ELF_HEADER_LEN, 0, function(err, bytesRead, _buf) {
                if (bytesRead < ELF_HEADER_LEN || err) {
                    log.silly("Packager#checkELFHeader()", "err:", err, ", bytesRead:", bytesRead);
                    log.silly("Packager#checkELFHeader()", "readBuf to parse ELF header is small or error occurred during reading file.");
                    return setImmediate(next);
                }

                if (!_isELF(_buf)) {
                    log.silly("Packager#checkELFHeader()", mainFile + " is not ELF format");
                } else {
                    log.silly("Packager#checkELFHeader()", mainFile + " is ELF format");
                    try {
                        const elfHeader = elfParser.parseHeader(_buf);
                        log.silly("Packager#checkELFHeader()", "elfHeader:", elfHeader);
                        ...
                        log.verbose("Packager#checkELFHeader()", "exception:", e);
                    }
                }
                log.verbose("Packager#checkELFHeader()", "machine:", self.architecture);
                fs.close(fd);
                setImmediate(next);
        });
    }
    ...
    function copyApp(next) {
        ...
        this.dataCopyCount++;
        log.info("Packager#copyApp()", "copy " + this.appDir + " ==> " + this.applicationDir);
    ...
    function copyAssets(next) {
        ...
        function _copyAssets(file, next) {
            log.verbose("Packager#copyAssets()#_copyAssets", "'" + file + "' will be located in app directory");
    ...
    function createPackageDir(next) {
        if (!this.rom) {
            ...
            log.verbose("Packager#createPackageDir", "directory: ", this.packageDir);
    ...
    function fillPackageDir(next) {
        if (!this.rom) {
            ...
            if (!this.pkgDir) {
                ...
                log.silly("Packager#fillPackageDir()", "generating package.json:" + data);
    ...
    function outputPackage(destination, next) {
        log.info("Packager#outputPackage()");
    ...
    function getFileSize(srcDir, next) {
        ...
        async.waterfall([
            ...
            if (!err && size) {
                log.verbose("Packager#getFileSize()", "Installed-Size:", size);
                self.size = size;
            }
            setImmediate(next, err);
        });
    ...
    function createDir(dstPath, next) {
        log.verbose("Packager#createDir()", "createDir:" + dstPath);
        mkdirp(dstPath, next);
    }

    function createControlFile(dstDir, encInfo, next) {
        const dstFilePath = path.join(dstDir, 'control');
        log.verbose("Packager#createControlFile()", "createControlFile:" + dstFilePath);
    ...
    function createSign(dstDir, dataTgzPath, next) {
        if ((!this.sign) || (!this.certificate)) {
            log.verbose("Packager#createSign()", "App signing is skipped");
            return setImmediate(next);
        }
        ...
        log.verbose("Packager#createSign()", "dataTgzPath : " + dataTgzPath + ", sigfile : " + sigFilePath);
        log.verbose("Packager#createSign()", "keyPath : " + keyPath + ", crtPath : " + crtPath);
    ...
    function createDebianBinary(dstDir, next) {
        ...
        log.verbose("Packager#createDebianBinary()", dstFilePath);
        fs.writeFile(dstFilePath, "2.0\n", next);
    }

    function makeTgz(srcDir, dstDir, next) {
        log.verbose("Packager#makeTgz()", "makeTgz " + dstDir + " from " + srcDir);
    ...
    function makeIpk(srcDir, next) {
        ...
        log.info("Packager#makeIpk()", "makeIpk in dir " + this.IpkDir + " file " + this.ipkFileName);
        ...
        pkgFiles.forEach(function (f) {
            ...
            arStream.append(s);
            if ((stat.size % 2) !== 0) {
                log.verbose("Packager#makeIpk()", 'adding a filler for file ' + f);
                arStream.append('\n');
            }
        }, this);
    ...
    function cleanupTmpDir(next) {
        if (this.noclean) {
            ...
            rimraf(this.tempDir, function(err) {
                log.verbose("Packager#cleanupTmpDir()", "removed " + this.tempDir);
                setImmediate(next, err);
            }.bind(this));
        }
    }
    ...
    function checkDirectory(options, directory, callback) {
        log.verbose("Packager#checkDirectory()", directory);
        ...
        if (fs.existsSync(path.join(directory, "appinfo.json"))) {// TODO: move to asynchronous processing
            this.appCount++;
            log.verbose("Packager#checkDirectory()", "FOUND appinfo.json, appCount " + this.appCount);
    ...
    function loadServiceInfo(next) {
        ...
        log.info("Packager#loadServiceInfo()", "num of serviceInfo: " + this.services.length);
        setImmediate(next);
    }
    ...
    function createServiceDir(next) {
        this.services.forEach(function(service) {
            ...
            getPkgServiceNames(service.serviceInfo).forEach(function(serviceName) {
                ...
                try {
                    log.info("Packager#createServiceDir()", "service dir:" + serviceDir);
    ...
    function addServiceInPkgInfo(next) {
        if (!this.rom) {
            ...
            try {
                ...
                log.silly("Packager#addServiceInPkgInfo()", "PACKAGEINFO:", pkginfo);
            } catch (err) {
                console.error(err);
                setImmediate(next, err);
            }
            ...
            if (validServiceCount > 0) {
                ...
                log.silly("Packager#addServiceInPkgInfo()", "Modified package.json: " + data);
                fs.writeFile(path.join(this.packageDir, "packageinfo.json"), data, next);
            } else {
                setImmediate(next);
            }
    ...
    function copyData(inDirs, forceCopy, next) {
        log.verbose("Packager#copyData()", "only run when force packaging");
    ...
    function rewriteFileWoBOMAsUtf8(filePath, rewriteFlag, next) {
        ...
        if (['UTF-8', 'ISO-8895-1'].indexOf(encodingFormat) === -1) {
            log.silly("Packager#rewriteFileWoBOMAsUtf8()", "current encoding type:" + encodingFormat);
    ...
    function encryptPackage(next) {
        if (this.rom || !this.encrypt) { // Do not encrypt when -rom option is given
            ...
            async.series([
                ...
                log.verbose("Packager#encryptPackage()", "success to encrypt pacakge");
                setImmediate(next);
            });
        }
    }
    ...
    function encryptIpk(dstPath, next) {
        log.verbose("Packager#encryptPackage()#encrypIpk()", "encrypt plain ipk to /encrypt/data");
        ...
        try {
            ...
            output.on('close', function() {
                log.verbose("Packager#encryptPackage()#encrypIpk()", "encrypted Ipk to " + encrypedIpkPath);
                setImmediate(next);
            }).
