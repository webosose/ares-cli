...
const Cli = (function() {
    ...
    function init() {
        ...
        try {
            ...
            if (result && result.resultValue) {
                ...
                log.verbose("cliAppData#init()", "schema check failure:", userDeviceListFile);
                log.verbose("cliAppData#init()", "result:", result);
                log.verbose("cliAppData#init()", "copy " + builtinDeviceListFile + " to " + userDeviceListFile);
                shelljs.cp('-f', builtinDeviceListFile, userDeviceListFile);
            }
        } catch (err) {
            log.verbose("cliAppData#init()", "err:", err);
            log.verbose("cliAppData#init()", "copy " + builtinDeviceListFile + " to " + userDeviceListFile);
            shelljs.cp('-f', builtinDeviceListFile, userDeviceListFile);
        }
    ...
    CliAppData.prototype = {
        ...
        setDeviceList: function(deviceListData, next) {
            log.info("cliAppData#setDeviceList()");
            try {
        ...
        resetDeviceList: function(next) {
            log.info("cliAppData#resetDeviceList()");
            try {
