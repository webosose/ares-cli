...
(function() {
    ...
    novacom.Resolver.prototype = {
        ...
        load: function(next) {
            ...
            async.waterfall([
                ...
                if (err) {
                    ...
                    log.silly("Resolver#load()", "devices:", resolver.devices);
                    setImmediate(next);
                }
            });

            function _replaceBuiltinSshKey(next) {
                log.silly("Resolver#load#_replaceBuiltinSshKey()");
            ...
            function _adjustList(next) {
                log.silly("Resolver#load#_adjustList()");
            ...
            function _loadString(next) {
                log.silly("Resolver#load#_loadString()");
        ...
        save: function(devicesData, next) {
            log.silly("Resolver#save()", "devicesData:", devicesData);
        ...
        getDeviceBy: function(key, value, next) {
            log.info("Resolver#getDeviceBy()", "key:", key, ", value:", value);
            ...
            if (devices.length < 1) {
                ...
                log.silly("Resolver#getDeviceBy()", "devices:", devices);
        ...
        modifyDeviceFile: function(op, target, next) {
            log.verbose("Resolver#modifyDeviceFile()", "op:", op, ", targt:", target);
            ...
            log.verbose("Resolver#modifyDeviceFile()", "matchedDevices:", matchedDevices);
            if (op === 'add') {
    ...
    novacom.Session.prototype = {
        ...
        begin: function(target, next) {
            log.verbose('Session#begin()', "target:", target.display);
            const self = this;
            ...
            if (!this.ssh) {
                ...
                this.ssh.on('connect', function() {
                    log.info('Session#begin()', "ssh session event: connected");
                });
                this.ssh.on('ready', _next.bind(this));
                this.ssh.on('error', _next.bind(this));
                this.ssh.on('end', function() {
                    log.info('Session#begin()', "ssh session event: end");
                });
                this.ssh.on('close', function(had_error) {
                    log.info('Session#begin()', "ssh session event: close (had_error:", had_error, ")");
                });
            ...
            function _clearSession() {
                log.verbose("Session#begin()", "clear Session");
                self.end();
                setTimeout(function() {
                    process.exit();
                }, 500);
         ...
         */
            ...
            log.info('Session#end()', "user-requested termination");
            if (this.ssh) {
                this.ssh.end();
            }
            return this;
        ...
        put: function(inPath, outPath, next) {
            log.info('Session#put()', "uploding into device:", outPath, "from host:", inPath);
            const self = this;
            let inStream;
            ...
            self.sftpPut(inPath, outPath, function(err) {
                if (err) {
                    ...
                    if (4 === err.code || 127 === err.code) {
                        log.info('Session#put()', "sftp is not available, attempt transfering file via streamPut");
                    ...
                    log.info('Session#put()', "sftpPut() :: done");
                    setImmediate(next);
                }
            });
        },
        ...
        streamPut: function(outPath, inStream, next) {
            log.info('Session#streamPut()', "streaming into device:" + outPath);
        ...
        get: function(inPath, outPath, next) {
            ...
            self.sftpGet(inPath, outPath, function(err) {
                if (err) {
                    log.verbose(err);
                    if (4 === err.code || 127 === err.code) {
                        log.info('Session#get()', "sftp is not available, attempt transfering file via streamPut");
        ...
        run_ssh: function(cmd, opt, stdin, stdout, stderr, next) {
            log.info('Session#run()', "cmd:" + cmd + ", opt:" + JSON.stringify(opt));
            if (typeof next !== 'function') {
                throw errHndl.getErrMsg("MISSING_CALLBACK", "next", util.inspect(next));
            }
            ...
            this.ssh.exec(cmd, opt, (function(err, chStream) {
                log.silly('Session#run()', 'exec cmd:' + cmd + ', opt:' + JSON.stringify(opt) + ', err:' + err);
                if (err) {
                    return setImmediate(next, err);
                }
                ...
                chStream.on('end', function() {
                    log.silly('Session#run()', "event EOF from (cmd:" + cmd + ")");
                ...
                chStream.on('exit', function(code, signal) {
                    log.silly('Session#run()', "event exit code:" + code + ', signal:' + signal + " (cmd:" + cmd + ")");
                ...
                chStream.on('close', function() {
                    log.silly('Session#run()', "event close (cmd:" + cmd + ")");
                    if (err === undefined) {
                        setImmediate(next);
                    }
                });
        ...
        runNoHangup_ssh: function(cmd, cbData, cbExit, next) {
            log.info('Session#runNoHangup()', "cmd=" + cmd);
        ...
        forward: function(devicePort, localPort, forwardName, next) {
            log.info('Session#forward()', "devicePort:", devicePort, ", localPort:", localPort);
            const session = this;
