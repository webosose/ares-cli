...
(function() {
    ...
    function makeExecError(cmd, code, signal) {
        ...
        if (code !== 0 || signal) {
            err = new Error();
            err.code = code;
            err.message = "Command '" + cmd + "' exited with code=" + code + " (signal: " + signal + ")";

            return errHndl.getSyscallErrMsg(err);
        }
        return err;
    }
    ...
    novacom.Resolver.prototype = {
        ...
        load: function(next) {
            ...
            function _loadString(next) {
                ...
                if (!Array.isArray(inDevices)) {
                    setImmediate(next, errHndl.changeErrMsg("INVALID_FILE"));
                    return;
                }
        ...
        _loadOne: function(inDevice, next) {
            ...
            if (typeof inDevice.privateKey === 'string') {
                ...
                setImmediate(next, errHndl.changeErrMsg("NOT_IMPLEMENTED", "webOS Pro device type handling"));
        ...
        setTargetName: function(target, next) {
            ...
            if (!name) {
                ...
                if (usbDevices.length > 1) {
                    return next(errHndl.changeErrMsg("CONNECTED_MULTI_DEVICE"));
                }
                ...
                if (usbDevices.length > 0 && usbDevices[0].name) {
                    ...
                    if (defaultTargets.length > 1) {
                        return next(errHndl.changeErrMsg("SET_DEFAULT_MULTI_DEVICE"));
        ...
        getDeviceBy: function(key, value, next) {
            ...
            if (devices.length < 1) {
                setImmediate(next, errHndl.changeErrMsg("UNMATCHED_DEVICE", key, value));
            } else {
        ...
        getSshPrvKey: function(target, next) {
            ...
            if (!name) {
                setImmediate(next, errHndl.changeErrMsg("EMPTY_VALUE", "DEVICE_NAME"));
                return;
            }
                ...
                function(targetDevice, next) {
                    ...
                    request.head(url, function(err, res) {
                        if (err || (res && res.statusCode !== 200)) {
                            return setImmediate(next, errHndl.changeErrMsg("FAILED_GET_SSHKEY"));
                        }
        ...
        modifyDeviceFile: function(op, target, next) {
            ...
            if (!target.name) {
                return setImmediate(next, errHndl.changeErrMsg("EMPTY_VALUE", "target"));
            }

            if (!Array.isArray(inDevices)) {
                return setImmediate(next, errHndl.changeErrMsg("INVALID_FILE"));
            }
            ...
            if (op === 'add') {
                if (matchedDevices.length > 0) {
                    return setImmediate(next, errHndl.changeErrMsg("EXISTING_VALUE", "DEVICE_NAME", target.name));
                }
                ...
                let errMsg;
                if (matchedDevices.length === 0) {
                    return setImmediate(next, errHndl.changeErrMsg("INVALID_VALUE", "DEVICE_NAME", target.name));
                }
                ...
                return setImmediate(next, errHndl.changeErrMsg("UNKNOWN_OPERATOR", op));
            }
    ...
    novacom.Session.prototype = {
        ...
        checkConnection: function(target, next) {
            ...
            if (target && target.host && target.port) {
                ...
                client.on('error', function(err) {
                    client.destroy();
                    setImmediate(next, errHndl.getSyscallErrMsg(err));
                });
                client.on('timeout', function() {
                    client.destroy();
                    if (!alive) {
                        setImmediate(next, errHndl.changeErrMsg("TIME_OUT"));
                    }
                });
            } else {
                setImmediate(next, null, target);
        ...
        begin: function(target, next) {
            ...
            if (this.target.privateKey === undefined && this.target.password === undefined) {
                return setImmediate(next, errHndl.changeErrMsg("NOT_EXIST_SSHKEY_PASSWD"));
            }
            ...
            function _next(err) {
                setImmediate(next, (err? errHndl.changeErrMsg(err) : err), this);
            }
            ...
            self.ssh.subsys('sftp', function(err, _stream) {
                ...
                _stream.once('data', function(data) {
                    ...
                    if (data.toString().match(regex)) {
                        const sftpError = errHndl.changeErrMsg("UNABLE_USE_SFTP");
                        sftpError.code = 4;
            ...
            self.sftpPut(inPath, outPath, function(err) {
                if (err) {
                    ...
                    if (4 === err.code || 127 === err.code) {
                        ...
                        const detailMsg = errHndl.changeErrMsg("NO_FREE_SPACE");
        ...
        run_ssh: function(cmd, opt, stdin, stdout, stderr, next) {
            ...
            if (!stdout) {
                ...
                setImmediate(next, errHndl.changeErrMsg("INVALID_VALUE", "stdout", util.inspect(stdout)));
            }
            ...
            if (!stderr) {
                ...
                setImmediate(next, errHndl.changeErrMsg("INVALID_VALUE", "stderr", util.inspect(stderr)));
            }
        ...
        runNoHangup_ssh: function(cmd, cbData, cbExit, next) {
            ...
            if (arguments.length < 2) {
                throw errHndl.changeErrMsg("MISSING_CALLBACK", "next");
            }
