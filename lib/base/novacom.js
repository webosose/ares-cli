...
(function() {
    ...
    novacom.Resolver.prototype = {
        ...
        load: function(next) {
            ...
            async.waterfall([
                ...
                if (err) {
                    ...
                    log.silly("novacom#load()", "devices:", resolver.devices);
                    setImmediate(next);
                }
            });

            function _replaceBuiltinSshKey(next) {
                log.silly("novacom#load()#_replaceBuiltinSshKey()");
            ...
            function _adjustList(next) {
                ...
                if (defaultFields.length < 1) {
                    log.silly("novacom#load()#_adjustList()", "rewrite default field");
        ...
        _loadOne: function(inDevice, next) {
            if (typeof inDevice.privateKey === 'string') {
                ...
                async.waterfall([
                    ...
                    if (err) {
                        log.verbose("novacom#_loadOne()", "Unable to find SSH private key named '" +
                ...
                if (!inDevice.password) {
                    log.silly("novacom#_loadOne()", "Regist privateKey : need to set a SSH private key in " +
        ...
        getDeviceBy: function(key, value, next) {
            log.info("novacom#getDeviceBy()", "key:", key, ", value:", value);
        ...
        modifyDeviceFile: function(op, target, next) {
            log.info("novacom#modifyDeviceFile()", "op:", op);
    ...
    novacom.Session.prototype = {
        ...
        begin: function(target, next) {
            log.verbose("novacom#begin()", "target:", target.display);
            const self = this;
            ...
            if (!this.ssh) {
                ...
                this.ssh.on('connect', function() {
                    log.info("novacom#begin()", "ssh session event: connected");
                });
                this.ssh.on('ready', _next.bind(this));
                this.ssh.on('error', _next.bind(this));
                this.ssh.on('end', function() {
                    log.info("novacom#begin()", "ssh session event: end");
                });
                this.ssh.on('close', function(had_error) {
                    log.info("novacom#begin()", "ssh session event: close (had_error:", had_error, ")");
                });
            ...
            function _clearSession() {
                log.verbose("novacom#begin()", "clear Session");
                self.end();
                setTimeout(function() {
                    process.exit();
                }, 500);
         ...
         */
            ...
            log.info('novacom#end()', "user-requested termination");
            if (this.ssh) {
                this.ssh.end();
            }
            return this;
        ...
        put: function(inPath, outPath, next) {
            log.info("novacom#put()", "uploding into device:", outPath, "from host:", inPath);
            const self = this;
            let inStream;

            log.verbose("novacom#put()", "sftpPut()::start");
            self.sftpPut(inPath, outPath, function(err) {
                if (err) {
                    log.verbose(err);
                    if (4 === err.code || 127 === err.code) {
                        log.info("novacom#put()", "sftp is not available, attempt transfering file via streamPut");
                    ...
                    log.info("novacom#put()", "sftpPut()::done");
                    setImmediate(next);
                }
            });
        },
        ...
        streamPut: function(outPath, inStream, next) {
            log.info("novacom#streamPut()", "streaming into device:" + outPath);
        ...
        sftpPut: function(inPath, outPath, next) {
            log.verbose('novacom#sftpPut()', 'host:' + inPath + ' => ' + 'device:' + outPath);
            const self = this;
                ...
                transfer: function(next) {
                    self.ssh.sftp(function(err, sftp) {
                        ...
                        writeStream.on('error', function(error) {
                            log.verbose('novacom#sftpPut()', "error:", error);
        ...
        get: function(inPath, outPath, next) {
            log.verbose("novacom#get()", "downloading into host:", outPath, "from target:", inPath);
            const self = this;

            log.verbose("novacom#get()", "sftpGet()::start");
            self.sftpGet(inPath, outPath, function(err) {
                if (err) {
                    log.verbose(err);
                    if (4 === err.code || 127 === err.code) {
                        log.info("novacom#get()", "sftp is not available, attempt transfering file via streamPut");
                    ...
                    log.verbose("novacom#get()", "sftpGet()::done");
                    setImmediate(next);
                }
            });
        },
        ...
        streamGet: function(inPath, outStream, next) {
            log.verbose('novacom#streamGet()', "streaming from device:" + inPath);
        ...
        sftpGet: function(inPath, outPath, next) {
            log.verbose("novacom#sftpGet()", "target:" + inPath + " => " + "host:" + outPath);
            const self = this;
            self._checkSftp(next);
            async.series({
                transfer: function(next) {
                    self.ssh.sftp(function(err, sftp) {
                        ...
                        readStream.on('error', function(error) {
                            log.verbose("novacom#sftpGet()", "error:", error);
                            setImmediate(next, error);
                        });
        ...
        run_ssh: function(cmd, opt, stdin, stdout, stderr, next) {
            log.info("novacom#run()", "cmd:" + cmd + ", opt:" + JSON.stringify(opt));
            if (typeof next !== 'function') {
                throw errHndl.getErrMsg("MISSING_CALLBACK", "next", util.inspect(next));
            }
            ...
            this.ssh.exec(cmd, opt, (function(err, chStream) {
                log.silly("novacom#run()", "exec cmd:" + cmd + ", opt:" + JSON.stringify(opt) + ", err:" + err);
                if (err) {
                    return setImmediate(next, err);
                }
                ...
                chStream.on('data', function(data, extended) {
                    ...
                    log.verbose("novacom#run()", "on data (" + extended + ")");
                    write[extended].bind(obj[extended])(data);
                }).stderr.on('data', function(data) {
                    log.verbose("novacom#run()", "on data (stderr)");
                    orgErrMsg = data.toString();
                });

                // manual pipe(): handle EOF
                chStream.on('end', function() {
                    log.silly("novacom#run()", "event EOF from (cmd:" + cmd + ")");
                ...
                chStream.on('exit', function(code, signal) {
                    log.silly("novacom#run()", "event exit code:" + code + ', signal:' + signal + " (cmd:" + cmd + ")");
                ...
                chStream.on('close', function() {
                    log.silly("novacom#run()", "event close (cmd:" + cmd + ")");
                    if (err === undefined) {
                        setImmediate(next);
                    }
                });
                ...
                if (stdin) {
                    stdin.pipe(chStream);
                    log.verbose("novacom#run()", "resuming input");
                }
            }));
        },
        ...
        runNoHangup_ssh: function(cmd, cbData, cbExit, next) {
            log.info("novacom#runNoHangup()", "cmd=" + cmd);
            ...
            this.ssh.exec(cmd, (function(err, _stream) {
                log.verbose("novacom#run()", "exec cmd:" + cmd + ", err:" + err);
                if (err) {
                    return setImmediate(next, err);
                }
                ...
                _stream.on('data', function(data) {
                    ...
                    log.verbose("novacom#runNoHangup()#onData", str);
                    if (cbData) cbData(data);
                }).stderr.on('data', function(data) {
                    const str = (Buffer.isBuffer(data)) ? data.toString() : data;
                    log.verbose("novacom#runNoHangup()#onData#stderr#", str);
                    if (cbData) cbData(data);
                });
                ...
                if (cbExit) {
                    _stream.on('exit', function(code, signal) {
                        log.verbose("novacom#runNoHangup()", "event exit code=" + code + ', signal=' + signal + " (cmd: " + cmd + ")");
        ...
        forward: function(devicePort, localPort, forwardName, next) {
            log.info("novacom#forward()", "devicePort:", devicePort, ", localPort:", localPort);
            const session = this;
            ...
            const localServer = net.createServer(function(inCnx) {
                log.info("novacom#forward()", "new client, localPort:", localPort);
                log.verbose("novacom#forward()", "new client, from: " + inCnx.remoteAddress + ':' + inCnx.remotePort);

                inCnx.on('error', function(err) {
                    log.verbose("novacom#forward()", "inCnx::error, err::" + err);
                });
                ...
                session.ssh.forwardOut("127.0.0.1" /* srcAddr*/ , inCnx.remotePort /* srcPort*/ , "127.0.0.1" /* dstAddr*/ , devicePort /* dstPort*/ , function(err, outCnx) {
                    if (err) {
                        console.log("novacom#forward()", "failed forwarding client localPort:",
                                    localPort, "(inCnx.remotePort:", inCnx.remotePort, ")=> devicePort:", devicePort);
                        log.warn("novacom#forward()", "failed forwarding client localPort:",
                                localPort, "=> devicePort:", devicePort);
                        inCnx.destroy();
                        return;
                    }

                    log.info("novacom#forward()", "connected, devicePort:", devicePort);
                    ...
                    inCnx.on('close', function(had_err) {
                        log.verbose("novacom#forward()", "inCnx::close, had_err:", had_err);
                        outCnx.destroy();
                    });
                    ...
                    outCnx.on('close', function(had_err) {
                        log.verbose("novacom#forward()", "outCnx::close, had_err:", had_err);
                    });
                });
            });
