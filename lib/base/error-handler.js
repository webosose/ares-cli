...
(function () {
    ...
    errMsgHdlr.getSyscallErrMsg = function(err) {
        console.log("errCode : "+ err.code, "errSyscall : " + err.syscall + " errMsg : " + err.message);

        const resultErr = [];
        let heading = "syscall",
            code = err.code;
        
        // hanle as "ssh exec" error
        if(!err.syscall) {
            heading = "ssh exec";
            code = err.message.trim();
        }

        const sysCallErr = new CLIError(heading, err.message.trim());
        if(sysCallErr.message){
            resultErr.push(sysCallErr);
        }

        const tipsErr = errMsgHdlr.changeErrMsg(code, err.path);
        if(tipsErr.message){
            resultErr.push(tipsErr);
        }
