...
(function () {
    const errMsgHdlr = {},
        ...
        ErrMsgMap = {
            ...
            "EACCES" : "Please check that can execute the command to the path",
            ...
            "ENOENT": "Please check the given path is valid",
            ...
            //ssh call
            "Command" : "The command cannot execute on target device",
            ...
            "NO_METAFILE" : "There are no meta files such as appinfo.json",
            "NO_ACCOUNT" : "account directory support is not yet implemented",
            "OVER_APPCOUNT" : "Only one application is supported",
            "INVALID_SERVICEID" : "ServiceID must start with package id",
            "FAILED_MINIFY" : "Failed to minify code",
    ...
    errMsgHdlr.getSshErrMsg = function(err) {
        console.log("errCode : "+ err.code, "errCAll : " + err.syscall + " errText : " + err.message);

        const resultErr = [];
        const sysCallErr = new CLIError("ssh exec", err.message.trim());
        if(sysCallErr.message){
            resultErr.push(sysCallErr);
        }

        const tipsErr = errMsgHdlr.changeErrMsg(sysCallErr.message, err.path);
        if(tipsErr.message){
            resultErr.push(tipsErr);
        }
    ...
    errMsgHdlr.getSyscallErrMsg = function(err) {
        ...
        const resultErr = [];
        const sysCallErr = new CLIError("syscall", err.message.trim());
        if(sysCallErr.message){
            resultErr.push(sysCallErr);
        }

        const tipsErr = errMsgHdlr.changeErrMsg(err.code, err.path);
        if(tipsErr.message){
            resultErr.push(tipsErr);
        }
