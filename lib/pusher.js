...
(function () {
        ...
        */
        ...
        async.waterfall([
            ...
            function(session, next) {
                ...
                getDestType(session, dstPath, function(err, type) {
                    ...
                    if (!err && type === TYPE.FILE && srcPaths.length > 1) {
                        err = errHndl.changeErrMsg("EXISTING_FILETYPE_PATH");
                    }
                    next(err);
                });
            },
            ...
            if (!err) {
                ...
                err = errHndl.getSyscallErrMsg(err);
            }
            next(err, result);
        });
    };
            ...
            function(stats, next) {
                if (stats.isDirectory()) {
                    ...
                    if (dstType === TYPE.FILE) {
                        return next(errHndl.changeErrMsg("EXISTING_FILETYPE_PATH"));
                    }
    ...
    function mkDir(session, dstPath, next) {
        async.series([
            ...
            if (err && err.code === 1) {
                err = errHndl.changeErrMsg("FAILED_CREATE_DIR");
            }
            next(err);
        });
    }
